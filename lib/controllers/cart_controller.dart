...
class CartController extends GetxController {
  ...
  Future<void> updateCart(CartItem item) async {
    ...
    final originalItem = cartItems.firstWhereOrNull((i) => i.id == item.id);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final updatedItem = cartItems.firstWhereOrNull((i) => i.id == item.id);
        if (updatedItem != null) {
          ...
          await fetchCart(); // Item not found, refresh cart
        }
      } else {
  ...
  void increaseAmount(String id) {
    final item = cartItems.firstWhereOrNull((item) => item.id == id);
    if (item == null || updatingItemIds.contains(id)) return;
  ...
  Future<void> placeOrder(
      String deliveryAddressId, String paymentMethod) async {
    if (!_isTokenValid(profileController.token.value)) {
      errorMessage.value = 'Session expired. Please log in again.';
      _showErrorSnackBar(errorMessage.value);
      return;
